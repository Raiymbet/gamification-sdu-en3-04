...
*/
$tournament_info = '';
$title = $description = $when_opened = $when_closed = $public = $id_groups = $id_teacher = $status = '';
$datetime_added = date("Y-m-d h:i:s");

echo $datetime_added;

if(isset($_POST['tournament_info'])){
    $tournament_info = json_decode($_POST['tournament_info'],true);
    $title = $_POST['tournament_info']['about_tournament']['name'];
    echo $tournament_info;
